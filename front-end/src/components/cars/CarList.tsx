...
import NoCarFound from "./CarNotFound";
...
const CarList = () => {
    ...
    // Update currentPage when router.query.page changes
    useEffect(() => {
        if (router.query.page) {
            setCurrentPage(Number(router.query.page));
        } else {
            setCurrentPage(1); // Reset to page 1 when no page query exists
        }
    }, [router.query.page]);
    ...
    return (
        <div className="min-h-screen flex flex-col">
            <main className="flex-grow max-w-7xl mx-auto px-4 pt-9">
                {cars.length > 0 ? (
                    <>
